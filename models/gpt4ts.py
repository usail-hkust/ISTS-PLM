...
class ists_plm(nn.Module):
    ...
    def __init__(self, opt):
        ...
        for i in range(2):
            gpt2 = GPT2Model_wope.from_pretrained('gpt2', output_attentions=True, output_hidden_states=True)
            bert = BertModel_wope.from_pretrained('bert-base-uncased', output_attentions=True, output_hidden_states=True)
            print(opt.te_model, opt.st_model)
            if(i==0):
                if opt.te_model == 'gpt':
                    gpt2.h = gpt2.h[:opt.n_te_gptlayer]
                    self.gpts.append(gpt2)
                elif opt.te_model == 'bert':
                    bert.encoder.layer = bert.encoder.layer[:opt.n_te_gptlayer]
                    self.gpts.append(bert)
            elif(i==1):
                if opt.st_model == 'gpt':
                    gpt2.h = gpt2.h[:opt.n_st_gptlayer]
                    self.gpts.append(gpt2)
                elif opt.st_model == 'bert':
                    bert.encoder.layer = bert.encoder.layer[:opt.n_st_gptlayer]
                    self.gpts.append(bert)
...
class istsplm_vector(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.gpt2 = GPT2Model_wope.from_pretrained('gpt2', output_attentions=True, output_hidden_states=True)
        self.gpt2.h = self.gpt2.h[:opt.n_te_gptlayer]
