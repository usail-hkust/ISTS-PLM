...
def Activity_get_seq_length(args, records):
	...
	lens = []
	missing_rate = []
	for b, (record_id, tt, vals, mask, mask_observed_tp) in enumerate(records):
		n_observed_tp = (mask_observed_tp==True).sum()
		max_input_len = max(max_input_len, n_observed_tp)
		max_pred_len = max(max_pred_len, len(tt) - n_observed_tp)
		lens.append(n_observed_tp)

		L, N = mask.shape
		missing_rate.append((L*N-mask.sum().item())/(L*N))
	lens = torch.stack(lens, dim=0)
	median_len = lens.median()
	print('missing_rate:', np.mean(missing_rate))
